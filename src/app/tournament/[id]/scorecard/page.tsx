  ...
  function getR2Foursomes(): {
    ...
    Object.entries(foursomeGroups).forEach(([gNum, playerIds]) => {
      ...
      results.push({
        ...
        allTeamSkins: skinsResult.teamSkins,
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        ...
                        {(() => {
                          ...
                          foursome.players.forEach((p) => {
                            const existing = teamGroups.find((g) => g.teamId === p.teamId)
                            if (existing) existing.players.push(p)
                            else teamGroups.push({ teamId: p.teamId, players: [p] })
                          })

                          // Cumulative skins through a given hole for a team
                          function cumulativeSkinsThrough(teamId: string, holeNumber: number): number {
                            let sum = 0
                            for (const h of holes) {
                              if (h.hole_number > holeNumber) break
                              const result = foursome.holeResults[h.hole_number]
                              if (result?.winner?.teamId === teamId) sum += result.skinsWon
                            }
                            return sum
                          }
                            ...
                            return (
                              <React.Fragment key={group.teamId}>
                                {group.players.map((player, i) => {
                                  const isLastPlayerInGroup = i === group.players.length - 1
                                  // Per-player stroke total across totHoles
                                  let total = 0
                                  let count = 0
                                  totHoles.forEach((h) => {
                                  ...
                                  return (
                                    <tr key={player.id} className={isLastPlayerInGroup ? "" : "border-b border-green-50"}>
                                      <td className={`px-2 py-2 text-xs font-semibold sticky left-0 bg-white truncate max-w-[3.5rem] ${isOurTeam ? "text-green-800" : "text-gray-400"}`}>
                                        {player.name.split(" ")[0]}
                                      </td>
                                      {holeRange.map((h) => {
                                        const score = r2AllScores.find((s) => s.player_id === player.id && s.hole_number === h.hole_number)
                                ...
                                {/* Team skins running-tally footer */}
                                <tr className={`bg-green-100 ${isLastGroup ? "" : "border-b-2 border-green-200"}`}>
                                  <td className={`px-2 py-1.5 text-[10px] font-bold sticky left-0 bg-green-100 whitespace-nowrap ${isOurTeam ? "text-green-800" : "text-gray-500"}`}>
                                    {/* Strip "Team " prefix so longer names fit (e.g. "Team Voldemor" -> "Voldemor") */}
                                    {teamName.replace(/^Team\s+/i, "")}
                                  </td>
                                  {holeRange.map((h) => {
                                    const running = cumulativeSkinsThrough(group.teamId, h.hole_number)
                                    const holeComplete = !!foursome.holeResults[h.hole_number]
                                    return (
                                      <td key={h.hole_number} className={`px-1 py-1.5 text-[11px] text-center font-semibold ${
                                        holeComplete ? (isOurTeam ? "text-green-700" : "text-gray-500") : "text-gray-300"
                                      }`}>
                                        {holeComplete ? (running % 1 === 0 ? running : running.toFixed(1)) : "–"}
                                      </td>
                                    )
                                  })}
                                  <td className={`px-2 py-1.5 text-[11px] font-bold text-center ${isOurTeam ? "text-green-900" : "text-gray-600"}`}>
                                    {teamTotal % 1 === 0 ? teamTotal : teamTotal.toFixed(1)}
                                  </td>
                                </tr>
                              </React.Fragment>
                            )
