...
const tabs = [
  ...
  {
    label: "Scores",
    hrefFn: (id: string) => `/tournament/${id}/score`,
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
  ...
  {
    label: "Card",
    hrefFn: (id: string) => `/tournament/${id}/scorecard`,
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
...
export default function BottomNav({ tournamentId }: BottomNavProps) {
  ...
  return (
    <nav className="sticky bottom-0 z-10 bg-white border-t border-green-200 shadow-[0_-2px_8px_rgba(0,0,0,0.06)]">
      <div className="max-w-md mx-auto flex">
        {tabs.map((tab) => {
          const href = tab.href || tab.hrefFn!(tournamentId)
